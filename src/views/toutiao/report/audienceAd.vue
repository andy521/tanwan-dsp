...
<template>
    <div>
        <span>选择广告组</span>
        <Select v-model="campaign_id" style="width:300px" filterable @on-change="getHourReporting();getaudienceReportingChart()">
            <Option v-for="item in campaign_list" :value="item.campaign_id" :key="this">{{ item.campaign_name }}</Option>
        </Select>
        <div v-show="campaign_id!=''" class="margin-top-10">
            <span>时间范围</span>
            ...
            <Table :data="list" :loading="loading" :columns="tableColumns" :size="tableSize" class="margin-top-10" ref="adtable" @on-sort-change="sortchange" stripe></Table>
            <Row class="margin-top-10">
                <Col span="10"> 表格尺寸
...
export default {
    ...
    methods: {
        ...
        getHourReporting(page) {
            ...
            if (this.get_account_id == "" || this.campaign_id == "") return;
            Axios.post("api.php", {
                action: "ttAdPut",
                opt: "audienceCampaignReporting",
        ...
        getaudienceReportingChart() {
            ...
            Axios.post("api.php", {
                action: "ttAdPut",
                opt: "audienceCampaignReportingChart",
                campaign_id: this.campaign_id,
                account_id: this.get_account_id,
