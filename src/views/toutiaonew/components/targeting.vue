...
export default {
    ...
    methods: {
        ...
        changetargeting(targeting) {
            if (targeting.district) {
                this.targeting.district = targeting.district; // 地域类型
            } else {
                this.targeting.district = "";
            }
            if (targeting.city) {
                this.targeting.city = targeting.city; // 地域id
            } else {
                this.targeting.city = [];
            }
            if (targeting.gender) {
                this.targeting.gender = targeting.gender; // 性别
            } else {
                this.targeting.gender = 'GENDER_UNLIMITED';
            }
            if (targeting.age) {
                this.age_model = "1";
                this.targeting.age = targeting.age; // 年龄
            } else {
                this.age_model = "0";
                this.targeting.age = [];
            }
            if (targeting.ad_tag) {
                this.ad_tag_model = "1";
                this.targeting.ad_tag = targeting.ad_tag; // 兴趣分类
            } else {
                this.ad_tag_model = "0";
                this.targeting.ad_tag = [];
            }
            if (targeting.interest_tags) {
                this.interest_tags_model = "1";
                this.targeting.interest_tags = targeting.interest_tags; // 兴趣关键词
            } else {
            ...
            if (targeting.platform) {
                ...
                this.targeting.platform = targeting.platform; // 平台
            } else {
            ...
            if (targeting.ac) {
                this.ac_model = "1";
                this.targeting.ac = targeting.ac; // 网络类型
            } else {
                this.ac_model = "0";
                this.targeting.ac = [];
            }
            if (targeting.carrier) {
                ...
                this.targeting.carrier = targeting.carrier; // 运营商
            } else {
            ...
            if (targeting.activate_type) {
                ...
                this.targeting.activate_type = targeting.activate_type;//新用户
            } else {
                this.activate_type_model = "0";
                this.targeting.activate_type = [];
            }
            if (targeting.app_behavior_target) {
                this.targeting.app_behavior_target = targeting.app_behavior_target;//APP行为定向
            }
            if (targeting.app_category) {
                this.targeting.app_category = targeting.app_category;//APP行为定向,分类集合
            }
            if (targeting.device_brand) {
                this.device_brand_model = "1";
                this.targeting.device_brand = targeting.device_brand;//受众手机品牌
            } else {
                this.device_brand_model = "0";
                this.targeting.device_brand = [];
            }
            if (targeting.article_category) {
                this.article_category_model = "1";
                this.advanced_options = true;
                this.targeting.article_category = targeting.article_category//受众文章分类
            } else {
