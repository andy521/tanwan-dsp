    ...
    <Card dis-hover shadow>
        <div class="head">
            <Row :gutter="10">
                ...
                <Col :xs="12" :md="4">
                    <!-- 媒体选择: -->
                    <Select placeholder="--全部媒体--" style="width:100%"  class="margin-bottom-10">
                        <Option v-for="item in media" :value="item.value" :key="item.value">{{ item.label }}</Option>
                    </Select>
                </Col>
                <Col :xs="12" :md="6">
                    <!-- 版本: -->
                    <Select placeholder="--全部版本--" style="width:120px"  class="margin-bottom-10">
                        <Option v-for="item in versionList" :value="item.value" :key="item.value">{{ item.label }}</Option>
                    </Select>
                    <Button type="success" @click="modal6 = true"  class="margin-bottom-10">自定义指标</Button>
                </Col>
                <Col :xs="8" :md="4" offset="2">
                ...
                <Col :xs="12" :md="2">
                    <Button icon="document-text"  @click="exportData()"  class="margin-bottom-10">下载报表</Button>
                </Col>
            </Row>
        </div>    
        
        <Table :columns="columns1" :data="list" ref="tableCsv" size="small" :height="height"></Table>
