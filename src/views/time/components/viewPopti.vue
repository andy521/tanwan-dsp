...
<script>
	export default {
		...
		watch: {
			checkAllGroup(val) {
				this.$emit('showbox', this.checkAllGroup)
			}
		},
		methods: {
			//自定义指标全选
			handleCheckAll() {
				...
				if(this.checkAll) {
					this.checkAllGroup = ['game_name', 'account_name', 'campaign_id', 'configured_status', 'site_id', 'impression', 'click', 'cost', 'click_cost', 'active', 'reg_total', 'reg_per', 'reg_cost', 'reg_imei_cost', 'login_cost', 'pay_num', 'pay_total', 'pay_per', 'reg_arpu', 'income_per'];
				} else {
					this.checkAllGroup = [];
				}
				this.callback();
