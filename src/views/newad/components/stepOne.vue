        ...
        <div v-if="tabsid==0">
            ...
            <div class="tabpane">
                <div v-if="campaign_id!=''">
                    ...
                    <div class="camitem">
                        <span>标的物类型</span>
                        <em v-if="campaign.product_type=='UNKNOWN'">未知类型</em>
                        <em v-for="item in ads_config.product_type" v-if="campaign.product_type==item.val_type">{{item.name}}</em>
                    </div>
                    <div class="camitem">
                        <span>开启状态</span>
                        <em v-for="item in ads_config.configured_status" v-if="campaign.configured_status==item.val_type">{{item.name}}</em>
                    </div>
                    <div class="camitem">
                        <span>投放速度模式</span>
                        <em v-for="item in ads_config.speed_mode" v-if="campaign.speed_mode==item.val_type">{{item.name}}</em>
                    </div>
                </div>
            </div>
            <div class="next_btn">
