	...
	export default {
		...
		methods: {			
            ...
            setFilter(){
                ...
                Axios.post('api.php',{action:'ucAdPut',opt:'getCreativeTemplates',adgroup_id:this.adgroup_id}).then( 
                    res=>{
                        if(res.ret == '1'){
                            let list = res.data,
                                s=[];
                            list.forEach(item=>{
                                s.push({'value':item.creativeTemplateId,'label':item.creativeTemplateName});
                            });
                            this.adstyle = s;
            ...
            deleteFun(){
                ...
                let account= this.checkId[0],creative= '[' + this.creativeids.join(',') + ']';
                this.deleteData(account,creative);
            },
